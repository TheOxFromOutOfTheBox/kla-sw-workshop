...
def write_to_output(content):
    with open(r"Results/Milestone2.txt","a+") as file:
        for each in content:
            file.write(f"{each}\n")
...
src=MyClass(r"Milestone_Input/Milestone 2/Source.txt")
src.get_header()
src.get_objects()

poi=MyClass(r"Milestone_Input/Milestone 2/POI.txt")
poi.get_header()
print(poi.header)
poi.get_objects()
...
shape=getDiff(shape_tofind)
found_idx=[]
for i  in range(len(all_shapes)):
    test_shape=getDiff(all_shapes[i])
    if(shape==test_shape):
        # print("Match")
        print(i)
        found_idx.append(i)
